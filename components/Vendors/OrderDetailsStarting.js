...
const OrderDetailsStarting = ({ order }) => {
  ...
  return (
    <>
          ...
          {order.status === "WAITING_FOR_ACCEPT" ? (
            ...
            <div>
              ...
              <div>
                <div className="grid grid-cols-2 mt-2 border-b pb-2 divide-x">
                  <div className="">
                    <p className="text-lg font-medium">Facilites</p>
                    <div className="text-sm max-h-[150px] overflow-y-auto">
                      {order.facilites?.length > 0 ? (
                        <div className="flex flex-wrap gap-x-4 mt-2 pr-4">
                          {order.facilites.map((item, index) => (
                            <span key={index} className="text-sm">
                              {item.title}
                              {order.facilites.length > index + 1 && ", "}
                            </span>
                          ))}
                        </div>
                      ) : (
      ...
      {/* <div className="md:hidden pt-4 font-roboto">
        <div className="flex justify-between items-center mb-[36px]">
          <div className="flex gap-4 items-center flex-1">
            ...
            <div className="text-sm flex-1">
              <Link href={`/feed/service/${order.service.slug}`}>
                <a>
              ...
              <div className="flex items-center justify-between flex-1 gap-4">
                <p className="text-gray-500 line-clamp-1 flex-1">
                  {order.service?.providerInfo?.position}
                </p>
        ...
        <div className="border-b border-[#fafafa] pb-2 text-center text-[14px] mb-[36px]">
          Bargaining service
        </div>
        ...
        <div className="flex justify-between mb-[36px]">
          <div className="text-sm">
            <p>ID : {order.id}</p>
            <p>Date : {format(new Date(order.createdAt), "MM-dd-yyyy")}</p>
          </div>
          <div className="scale-75 origin-top-right h-max -mt-2">
            <Barcode
              width={1.2}
              height={40}
              text={order.id.toUpperCase()}
        ...
        <div className="mb-6">
          ...
          {order.status === "WAITING_FOR_ACCEPT" ? (
            <div className="text-red-500">
              please give clear instructions to the seller via chat. They will
              add the services to your receipt as per your requirements.
            </div>
          ) : (
            <div>
              <p className="mb-3">{order.gigTitle}</p>
        ...
        <div className="pb-6 border-b border-[#fafafa]">
          <p className="text-[20px] mb-3">Extra Facilites</p>
          {order.facilites?.length > 0 ? (
            <div className="flex flex-wrap gap-x-4 mt-2 pr-4">
              {order.facilites?.map((item, index) => (
                <span key={index} className="text-sm">
        ...
        <div className="mt-6 border-b border-[#fafafa] pb-6">
          <div className="w-[220px] mx-auto">
            <p className="text-[20px] text-center">Price</p>
            <div className="flex justify-between mt-4">
              <p className="text-gray-500 ">Your Offer</p>
              <p className="text-gray-500 ">{order.offerPrice}৳</p>
            </div>
            <div className="flex justify-between mt-2">
              <p className="text-gray-500 ">Duty Fee {order.dutyFee * 100}%</p>
              <p className="text-gray-500 ">
                +{dutyFee(order.dutyFee, order.offerPrice)}৳
              </p>
            </div>
            <div className="flex justify-between mt-4">
              <p className="text-[20px] font-medium">You Pay</p>
        ...
        <div className="mt-6 border-b border-[#fafafa] pb-6">
          <p className="text-[20px] text-center mb-2">Delivery Date</p>
          <p className="text-gray-500 text-center mb-2 flex items-center gap-4 justify-center">
            <span>{order.deliveryDateFrom}</span> to
            <span>{order.deliveryDateTo}</span>
          </p>
          <div className="flex justify-end mt-4">
            <DeliveryLearnMoreUser />
          </div>
        </div>

        <div className="mt-6 border-b border-[#fafafa] pb-6">
          <p className="text-[20px] text-center mb-2">Payment Status</p>
          <p className={`capitalize text-center`}>{formatPaymentStatus()}</p>
        ...
        <div className="mt-6 border-b border-[#fafafa] pb-6">
          <p className="text-[20px] text-center mb-2">Service Status</p>
          <p className={`capitalize text-center`}>{formatOrderStatus()}</p>
          {order.paymentStatus === "DUE" && order.status === "ACCEPTED" && (
            <p className="text-[14px] mt-2 text-center text-[#7566FF]">
              Payment pending
            </p>
          )}
          {order.status === "DELIVERED" && (
            <div className="flex justify-end mt-4">
              <DeliveredStatusLearnMoreUser />
            </div>
          )}
          {order.requestedDeliveryDate && (
            <div className="text-base flex items-center justify-center gap-2 flex-wrap flex-col mt-4">
              <span className="py-2">
              ...
              <div className="flex justify-end gap-6 w-full mt-2">
                <button
                  onClick={() => acceptOrRejectNewDeliveryDateFn("REJECT")}
                  className={`btn btn-outline border-black/20 btn-sm capitalize ${
                    isLoading && "loading"
                  }`}
                >
                  Cancel
                ...
                <button
                  onClick={() => acceptOrRejectNewDeliveryDateFn("ACCEPT")}
        ...
        {order.proofImage && (
          <div className="mt-6 pb-6">
            <p className="text-[20px] mb-2">Delivery file</p>
            <p className="mb-2">{order.proofText}</p>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className=" line-clamp-1">
                  Image.{getUrlExtension(order.proofImage || "")}
                </span>
              </div>
              <a
                download
        ...
        <div className="mt-6 pb-6">
          <p className="text-[20px] mb-4">Instruction</p>
          {order.description ? (
            <p className="text-sm mb-4">{order.description}</p>
          ) : (
            <p className="text-gray-400 mb-4">No Instruction message found</p>
          )}
          {order.attachment && (
            <a
              download
            ...
            >
              <div className="flex items-center gap-2">
                <AiOutlineFileImage className="text-2xl" />
                <span className=" line-clamp-1">
                  Image.{getUrlExtension(order.attachment || "")}
                </span>
              </div>
              <a
                download
        ...
        <div className="mt-[36px]">
          <div className="flex flex-col gap-3 justify-end items-center">
            {order.delivered && order.status !== "COMPLETED" && (
              <div className="text-center w-full">
                <p className="mb-2">
                  Click 'Recived' or Auto-Receive in 72 Hours
                </p>
                <ReceivedButton mobile orderId={order.id} />
              </div>
            )}
            ...
            {order.status === "ACCEPTED" && (
              <button
                onClick={() => makePaymentFn()}
                className={`btn rounded btn-primary btn-block capitalize font-medium ${
                  isLoading && "loading"
                }`}
              >
                Pay Now
            ...
            {order.status !== "CANCELLED" &&
              ...
              !order.paid && (
                <button
                  ...
                  className={`btn rounded btn-block border-black/20 btn-outline capitalize font-medium`}
                >
                  Cancel Order
                </button>
              )}
                ...
                {!order.review && (
                  <div className="w-full">
                    <button
                      ...
                      className="btn rounded btn-block btn-primary capitalize mt-2"
                    >
                      Give Review
                    </button>
                  </div>
